...
for s in schools:
    for pid, ys in s['scores'].items():
        row = {'name': s['name'], 'zs_code': s.get('zs_code'),
               'level': s.get('level'), 'tags': s.get('tags') or ''}
        has = False
        for y in YEARS:
            wu = ys.get(y) or {}
...
HDR = ['学校名称', '国标码', '层次', '标签', '2025物理类', '2025历史类',
       '2024物理类', '2024历史类', '2023物理类', '2023历史类']
KEYS = ['name', 'zs_code', 'level', 'tags', '2025_wl', '2025_ls',
        '2024_wl', '2024_ls', '2023_wl', '2023_ls']
...
for pid in provs:
    ...
    Wd = {1: 28, 2: 9, 3: 7, 4: 36}
    for ci in range(1, len(HDR)+1):
        ws.column_dimensions[openpyxl.utils.get_column_letter(ci)].width = Wd.get(ci, 10)
    ws.freeze_panes = 'A2'; ws.auto_filter.ref = ws.dimensions
    wb.save(base + '.xlsx')

    L = ['# ' + pn + ' · 院校录取分数线（2023–2025）\n',
         '> 数据来源：gaokao.cn · 院校最低分（物理类/历史类）· 共 ' + str(len(rows)) + ' 所院校\n',
         '\n| 学校 | 国标码 | 层次 | 标签 | 2025物理 | 2025历史 | 2024物理 | 2024历史 | 2023物理 | 2023历史 |',
         '|---|---|---|---|---|---|---|---|---|---|']
    for r in rows:
        L.append('| ' + ' | '.join([m(r['name']), m(r['zs_code']), m(r['level']), m(r['tags']),
...
print('OK provinces=' + str(len(made)) + ' rows=' + str(total))
